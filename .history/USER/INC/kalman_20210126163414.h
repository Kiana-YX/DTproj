 ...
 * @LastEditTime: 2021-01-26 16:34:14
...
typedef struct
{
	const float Q;	   					//预测的噪声最小协方差
	const float R;     					//测量的噪声最小协方差
	const float x_last,x_mid,x_now;  	//上次的最优值，本次测量值
	const float p_last,p_mid,p_now;  	//上次协方差，本次预测协方差
	const float kg;            			//增益
	const float A;             			//x(n)=Ax(n-1)+u(n)  系统转移矩阵
	const float B;						//控制矩阵 
	const float H;             			//z(n)=Hx(n)+v(n),z(n)为采样实测值，x（n）为预测值，H为观测转移矩阵	
	const float best;
}Kalman_Filter;
