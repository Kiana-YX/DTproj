...
short Linear_Fitting(SPI_TypeDef *SPIx)//SPI1为1号DT35，SPI2为2号DT35
{
	...
	#if DT_SELECT == 0x03		//L2 李茂二号板
	if(SPIx==SPI2)
	{
		distance2.kalmaninput=getAveragingData(&kalman_data2);
		distance2.temp=kalman_filter(&kalman_fliter2,distance2.kalmaninput);
		distance2.real=(3.394e-07)*distance2.temp*distance2.temp+0.4368*distance2.temp-2207;
		distance.send=(u26)(distance2.real+0.5);
	}
	return 0;
	#endif
	
	#if DT_SELECT == 0x04		//F4，2号板，QA2
	if(SPIx==SPI2)
	{
		distance2.kalmaninput=getAveragingData(&kalman_data2);
		distance2.temp=kalman_filter(&kalman_fliter2,distance2.kalmaninput);
